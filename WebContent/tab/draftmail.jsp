...
function deleteMail(){
	$(":checkbox[name='messageUID']").each(function(){
		if(this.checked){
 	document.getElementById("my").action ="thoroughDeleteC";
...
function deleteEmail(){
	$(":checkbox[name='messageUID']").each(function(){
		if(this.checked){		
		 	document.getElementById("my").action ="deleteMailD";
			document.forms[0].submit(); 				
		}	
	});
	}
...
<form id="my" name="my" action="" method="post">
                   ...
                   草稿箱:(共<span><s:property value="mailInfo.messageCount"/></span>封) 
                     ...
                     <span>
                      ...
                      <span>                    
                        ...
                        <s:else><s:property value="totalPageCount"/></s:else>页                     
                         <s:if test="index>1"> 
                         <a href="draftMail.action?index=1">首页</a>
                        </s:if>                     
                        <s:else>
                        <font color='#666666'>首页</font>
                        </s:else>
                        <s:if test="index>1"> 
                         <a href="draftMail.action?index=<s:property value='index-1'/>">上一页</a>
                        </s:if>                     
                        <s:else>
                        <font color='#666666'>上一页</font>
                        </s:else>
                        <s:if test="index<totalPageCount">
                         <a href="draftMail.action?index=<s:property value='index+1'/>">下一页</a>
                        </s:if>
                        <s:else>
                        <font color='#666666'>下一页</font>
                        </s:else>
                        <s:if test="totalPageCount > 0 && index != totalPageCount">                      
                         <a href="draftMail.action?index=<s:property value='totalPageCount'/>">末页</a> 
                        </s:if>
                        <s:else>
...
<form action="getmailBeanD" method="post">
